...
public class VideoDataMediator {
    ...
    public static final String STATUS_UPLOAD_ERROR =
        "Upload failed";
    /**
     * Status code to indicate download failed
     */
    private static final String DOWNLOAD_FAILED =
            "Download failed";
    private static final String DOWNLOAD_SUCCESSFUL = "Download Successful";
    private static final String SAVING_FAILED = "Unable to save video on device";
     ...
     * @param id
     */
    public String downloadVideoData(Context context,String videoName,long id){
        Response response;
        InputStream videoDataStream = null;
        try{
             response = mVideoServiceProxy.getData(id);
             videoDataStream = response.getBody().in();
        }catch(Exception ex){
            return DOWNLOAD_FAILED;
        }
        try{
             VideoStorageUtils.storeVideoInExternalDirectory(context,response,videoName);
        }catch (Exception ex){
